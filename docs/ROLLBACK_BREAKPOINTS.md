...
*Last updated: 2026-09-25 (BP-25 to BP-30, BP-32 and BP-33 live; BP-31 pushed; BP-34 committed) · working branch `final` (GitHub default) · repo `A1MITG/NewsLetter-CXO`
...
|---|---|---|---|---|---|---|
| BP-34 | `e6a9418` | 2026-09-25 15:08 | Committed | RowArrows | Round arrow buttons on the left and right edges of the engine tiles, People Movers and Leaders on Record replace the horizontal scrollbars under those rows (now hidden; swipe, trackpad and keyboard scrolling still work). A click pages one screenful of whole cards; an arrow shows only when there are cards that way, with a soft fade on the white rows; the drifting engine row pauses six seconds for its arrows (`signal:pause`). Executive Pulse is unchanged. Pages regenerated from the source; data untouched. Tests: `tests/test_row_arrows.py`. | `git revert e6a9418`, then rebuild and republish |
