...
*Last updated: 2026-09-24 (BP-31, BP-32 committed; BP-27 to BP-29 live; BP-25, BP-26 and BP-30 pushed) · working branch `final` (GitHub default) · repo `A1MITG/NewsLetter-CXO`
...
|---|---|---|---|---|---|---|
| BP-32 | `7a6d687` | 2026-09-24 12:13 | Committed | TodaysBrief | The closer's two dead buttons ("View Today's Brief" → `#brief`, which nothing carries; "Explore Signals" → the site's own front door in a new tab) become one, "Read Today's Brief" → `/brief`. Today's Brief is a text-only page: one section per tile in the page's tile order, each with the five stories that tile cycles, as a linked headline, its source and one line from the feed's description. No images, no scripts. New `app/analysis/brief.py` and `app/templates/brief.html`; the static build writes `public/brief.html` and the workflow publishes it to `signals-deploy` (cleanUrls serves `/brief`); Flask's `/brief` renders it from the cache as it stands (new `store.get_cached_articles()`, never scrapes). The old five-lens page `app/templates/index.html` is no longer served. Pages regenerated from the source; data untouched. Tests: `tests/test_todays_brief.py`. Goes live with the first build after it is pushed. Not on `command-center-deploy`, which has no cleanUrls. | `git revert 7a6d687`, then rebuild and republish |
| BP-31 | `a12b28c` | 2026-09-24 12:03 | Committed | CICorpusSkip | CI (*build-and-test*, "Run tests") had failed on every push since 2026-09-23. The runner has no `instance/articles_cache.json`, but the first test to call `/api/command-center` (`TestLiveEndpoint`) scraped the live feeds and wrote one, so every corpus test after it ran on that day's scrape instead of skipping, and the two known live-corpus failures (`test_single_keyword_rate_is_low_on_live_corpus`, `test_ranking_is_monotonic_among_leads`) failed the run (inferred: the run log needs a GitHub sign-in). The five classes that call the endpoint now take `@pytest.mark.usefixtures("raw_cache")` and skip without a cache; their five template-only tests move unchanged into `…Markup` classes so they still run on CI. Tests only: no assertion, app or page change, so it never goes live. On branch `claude/ci-test-fix`, a fast-forward of `final`; CI turns green once it is pushed. First committed as BP-30 (`7f4388a`) and renumbered, unpushed, when BP-30 Manifesto landed first. | `git revert a12b28c` |
