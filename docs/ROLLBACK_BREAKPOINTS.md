...
*Last updated: 2026-09-24 (BP-25 to BP-28 pushed) · working branch `final` (GitHub default) · repo `A1MITG/NewsLetter-CXO`
...
|---|---|---|---|---|---|---|
| BP-28 | `6e8330f` | 2026-09-24 10:12 | Pushed | BuilderLabel | "About the Founder" is now "About the Builder": the nav link and the section's eyebrow. Ids, classes and the image path keep "founder", so `#founder` links still land; the portrait's alt text is unchanged. Test: `test_labelled_about_the_builder`. Goes live with the next build. | `git revert 6e8330f`, then rebuild and republish |
| BP-27 | `bcc74ff` | 2026-09-24 10:09 | Pushed | WorldTimeLine | One thin full-width line between the nav and the hero: Tokyo, New Delhi, Dubai and New York, each with its flag and local date and time (browser time zones, no data), then the last refresh from `_meta.built_at` and `refresh_slots_ist` ("Updated … · Next …" in IST; amber "… update delayed" when a scheduled refresh is more than 90 min late; hidden without `built_at`). Replaces the engine band's "Sources checked" stamp, which on the static site always said "moments ago". `/api/command-center` sends `built_at` too. Pages regenerated from the source; the data file untouched. Tests: `tests/test_world_time_line.py`, plus a `built_at` check in `test_freshness_gate.py`. Goes live with the next build; the refresh cell appears once a build writes `built_at` (BP-26). | `git revert bcc74ff`, then rebuild and republish |
...
- **Revert newest first.** BP-02/03, BP-04, BP-06, BP-08, BP-10 to BP-23, BP-27 and BP-28 all touch the Command Center page (`command_center_source.html`, the template, `public/`). Reverting an older one on its own will likely conflict.
